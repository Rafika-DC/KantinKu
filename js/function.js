  ...
  <span class="visually-hidden">Tunggu sebentar...</span>\
        ...
        success: function (data) {
            ...
            if (data.add_class != null) {
                for (var a = 0; a < data.add_class.length; a++) {
                    if ($(data.add_class[a].target)) {
                        $(data.add_class[a].target).addClass(data.add_class[a].class);
                    }
                    
                }
            }
            if (data.remove_class != null) {
                for (var a = 0; a < data.remove_class.length; a++) {
                    if ($(data.remove_class[a].target)) {
                        $(data.remove_class[a].target).removeClass(data.remove_class[a].class);
                    }
                }
            }
